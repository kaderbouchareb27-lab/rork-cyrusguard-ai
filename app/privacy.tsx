...
export default function PrivacyScreen() {
  ...
  const content = language === 'fr' ? {
    ...
    sections: [
      ...
      { heading: '3. Traitement des données par l\'intelligence artificielle', body: 'CyrusGuard AI utilise l\'API OpenAI (modèle GPT-4o) pour analyser le contenu que vous soumettez (images, textes, URLs). Les données sont envoyées de manière sécurisée (chiffrement TLS) aux serveurs d\'OpenAI situés aux États-Unis, uniquement pour l\'analyse de fraude. OpenAI ne conserve pas les données transmises via l\'API au-delà du traitement immédiat et ne les utilise pas pour entraîner ses modèles. Aucune donnée personnelle identifiable n\'est volontairement transmise à OpenAI. Vous êtes informé de ce traitement avant votre première analyse via une fenêtre de consentement explicite.' },
    ...
    sections: [
      ...
      { heading: '3. Third-Party AI Data Processing', body: 'CyrusGuard AI uses the OpenAI API (GPT-4o model) to analyze the content you submit (images, texts, URLs). Data is securely transmitted (TLS encryption) to OpenAI servers located in the United States, solely for fraud analysis purposes. OpenAI does not retain data transmitted via the API beyond immediate processing and does not use it to train its models. No personally identifiable information is intentionally transmitted to OpenAI. You are informed of this processing before your first analysis via an explicit consent dialog.' },
